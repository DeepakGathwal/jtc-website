...
export async function POST(req){
  ...
  if(!redisdata){
    const query =  `Select batch.id,TIME_FORMAT(batch.time_to, '%h:%i %p') as time_to,TIME_FORMAT(batch.time_from, '%h:%i %p') as time_from, (CASE WHEN batch.date < CURDATE() Then 'Sold Out' WHEN batch.date BETWEEN CURDATE() AND (NOW() + INTERVAL 7 DAY) Then 'Filling Fast' ELSE 'Available' END) as status ,Date_Format(batch.date, '%d') as date, Date_Format(batch.date, '%M %y') as monthYear,cource.name as cource, batch.week_days, CASE WHEN EXTRACT(DAY FROM batch.date) IN (11, 12, 13) THEN 'th' WHEN EXTRACT(DAY FROM batch.date) % 10 = 1 THEN 'st' WHEN EXTRACT(DAY FROM batch.date) % 10 = 2 THEN 'nd' WHEN EXTRACT(DAY FROM batch.date) % 10 = 3 THEN 'rd' ELSE 'th' END AS formatted_date from jtc_batches as batch INNER Join jtc_courses as cource On cource.id = batch.cource_id WHERE batch.deleted_by = '0' && cource.link = '${course}' && cource.deleted_by = '0'`
